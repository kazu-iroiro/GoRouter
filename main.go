...
func main() {
	...
	addr := flag.String("addr", "0.0.0.0:8080", "Server listen/connect address (UDP)")
	lines := flag.Int("lines", 2, "Number of connection lines (Client mode only)")
	
	mtu := flag.Int("mtu", 1280, "Virtual Interface MTU") 
	fragSize := flag.Int("frag", 1300, "Fragmentation size")
	...
	routerMode := flag.Bool("router", false, "Enable IP forwarding and NAT (Client only)")
	...
	if *mode == "client" && *ifaces != "" {
		...
		if count > *lines {
			log.Printf("[Config] Auto-adjusting connection lines: %d -> %d", *lines, count)
			*lines = count
		}
	}
	...
	if *mode == "server" {
		...
		if *routerMode {
			if err := setupRouterMode(iface.Name()); err != nil {
				...
				log.Println("[INFO] Router mode enabled (IP Forwarding + NAT).")
			}
		}

		if *redirectGw {
			if *gatewayIP == "" {
				log.Fatal("Error: -gw is required when using -redirect-gateway.")
			}
...
func (q *QuicStreamConn) Write(b []byte) (n int, err error) { return q.Stream.Write(b) }
func (q *QuicStreamConn) Close() error                      { 
	// Streamを閉じるとそのストリームだけ終了。接続自体は維持される可能性があるが、
	// アプリ仕様上1接続1ストリームとして扱うため、Stream Closeで十分。
...
func generateServerTLSConfig() *tls.Config {
	...
	return &tls.Config{
		...
		NextProtos:   []string{"bonding-quic"},
	}
}
...
func runServer(iface *water.Interface, addr string, fragSize int) {
	...
	log.Printf("[Server] Listening on UDP %s (QUIC)", addr)
	...
	for {
		...
		go func(qConn quic.Connection) {
			...
			// net.Conn互換ラッパーに包む
			netConn := &QuicStreamConn{Stream: stream, Conn: qConn}

			// 既存のハンドシェイク処理 (RSA署名検証)
			if err := serverHandshake(netConn); err != nil {
				log.Printf("[Auth Failed] %v", err)
				netConn.Close()
				return
			}
			log.Printf("[Auth Success] Client: %s", netConn.RemoteAddr())

			wrapper := &ConnectionWrapper{
				Conn: netConn, Enc: gob.NewEncoder(netConn), Alive: true, BaseWeight: 10, RTT: 10 * time.Millisecond,
			}
...
func runClient(iface *water.Interface, serverAddr string, numLines int, fragSize int, weightStr string, ifaceStr string, cleanupFunc func()) {
	...
	for i := 0; i < numLines; i++ {
		// 特定のインターフェースにバインドするためのUDP Connを作成
		var udpConn *net.UDPConn
		
		if i < len(bindIfaces) && strings.TrimSpace(bindIfaces[i]) != "" {
			targetIface := strings.TrimSpace(bindIfaces[i])
			if localIP, err := resolveInterfaceIP(targetIface); err == nil {
				addr := &net.UDPAddr{IP: localIP, Port: 0}
				uc, err := net.ListenUDP("udp", addr)
				if err != nil {
					log.Printf("[Line %d] UDP Bind Error: %v", i, err)
				} else {
					udpConn = uc
					log.Printf("[Line %d] Bind UDP: %s (%s)", i, targetIface, uc.LocalAddr())
				}
			} else {
				log.Printf("[Line %d] Interface Resolve Error: %v", i, err)
			}
		}

		// UDPConnを指定してQUIC接続 (バインドしていない場合はnilで自動選択)
		...
		if err != nil {
			log.Printf("[Line %d] QUIC Dial Failed: %v", i, err)
			continue
		}
		...
		if err := clientHandshake(netConn); err != nil { 
			log.Printf("[Line %d] Handshake Failed: %v", i, err)
			netConn.Close()
			continue
		}
		
		log.Printf("[Line %d] Connected (QUIC).", i)
		successCount++

		cw := &ConnectionWrapper{
			ID: i, Conn: netConn, Enc: gob.NewEncoder(netConn), Alive: true, BaseWeight: manualWeights[i], RTT: 100 * time.Millisecond,
		}
		conns[i] = cw
...
func networkToTunLoop(iface *water.Interface, pktChan <-chan Packet) {
	...
	for {
		...
		case pkt := <-pktChan:
			...
			// リセット検出
			if pkt.SeqID == 0 {
				log.Println("[INFO] SeqID Reset detected. Clearing buffers.")
				nextSeqID = 0
				buffer = make(map[int64]Packet)
				ipPacketBuffer.Reset()
				initialized = true
...
func setupRoutes(tunName, serverIP string, gatewayIPs []string, bindIfaces []string) error {
	...
	for i, gw := range gatewayIPs {
		...
		if err != nil {
			// エラーならこの回線はスキップ（QUICのDial自体は試行されるかもしれないが、ルーティング設定はできない）
			continue
		}
		// QUICはUDPなのでnet.IPが返る
		srcIP := localAddr.String()
