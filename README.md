...
# TUNからのトラフィックを物理NIC(eth0など)から出ていく際にNATする
...
# デバッグログを確認したい場合
sudo ./bonding -mode server -pub public.pem -vip 10.0.0.1/24 -addr :8080 -debug
```
...
sudo ./bonding -mode client -priv private.pem -vip 10.0.0.2/24 -addr (Server_IP):8080 -lines 2 -ifaces "NIC_NAME1, NIC_NAME2" -redirect-gateway -gw (Default_GateWay) -debug
